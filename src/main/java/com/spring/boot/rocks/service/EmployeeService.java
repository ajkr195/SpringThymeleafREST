...
import com.spring.boot.rocks.model.EmployeeComparators;
import com.spring.boot.rocks.model.paging.Column;
import com.spring.boot.rocks.model.paging.Order;
...
public class EmployeeService {
	...
	public PageArray getEmployeesArray(PagingRequest pagingRequest) {
		pagingRequest.setColumns(Stream.of("name", "position", "office", "start_date", "salary").map(Column::new)
		...
		pageArray.setData(employeePage.getData().stream().map(this::toStringList).collect(Collectors.toList()));
		return pageArray;
	}
	...
	private List<String> toStringList(Employee employee) {
		return Arrays.asList(employee.getName(), employee.getPosition(), employee.getOffice(),
				sdf.format(employee.getStartDate()), employee.getSalary().toString());
	}
	...
	private Page<Employee> getPage(List<Employee> employees, PagingRequest pagingRequest) {
		List<Employee> filtered = employees.stream().sorted(sortEmployees(pagingRequest))
				.filter(filterEmployees(pagingRequest)).skip(pagingRequest.getStart()).limit(pagingRequest.getLength())
				.collect(Collectors.toList());
	...
	private Predicate<Employee> filterEmployees(PagingRequest pagingRequest) {
		...
		return employee -> employee.getName().toLowerCase().contains(value)
				|| employee.getPosition().toLowerCase().contains(value)
